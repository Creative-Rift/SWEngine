...
inline Cpt &sw::AManager<Cpt>::createComponent(const boost::uuids::uuid& uuid, Args... values)
{
    if (!m_scene.hasGameObject(uuid))
        throw sw::Error("GameObject doesn't exist");
    //if (m_components.contains(gameObjectName))
    /// TODO: add Log

    auto it = m_components.try_emplace(uuid, std::make_shared<Cpt>(m_scene.getGameObjectById(uuid), values...));
    if (it.second)
        m_componentsLayers.emplace_front(std::make_pair(0, uuid));
